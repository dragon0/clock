...
class Clock:
    ...
    def _draw_hand(self, color, center, width, angle, length):
        cx, cy = center
        ...
        right = radians(90)
        circle_theta = 90 - (360 / length * angle)
        circle_theta = radians(circle_theta)
        pygame.draw.arc(self._screen, color, box, circle_theta, radians(90), 1)

        line_theta = 180 - (360 / length * angle)
        line_theta = radians(line_theta)
        x = cx + (width/2 * sin(line_theta))
        y = cy + (width/2 * cos(line_theta))
